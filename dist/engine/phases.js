import { chooseAiAction, chooseAiDeckCardForGodDraw, chooseAiReserveCard, shouldAiUseGodDrawSlot, shouldAiUseJumpSlot } from "./ai.js";
import { calculateAdvantage, clamp, getAdvantageBreakdown, getSlotGain, groupDeckChoices, shuffle } from "./rules.js";
const PLAYER_ID = "P1";
const AI_ID = "P2";
function spendBurstSlot(player, slot, amount) {
...
function resetTurnScopedFlags(player) {
...
function applyTurnStartPassives(game, playerId) {
    const player = game.getPlayer(playerId);
    const character = game.getCharacter(player.character);
    if (character.passive.key === "loseOneSlotAtTurnStart") {
        const slotToReduce = player.jumpSlot >= player.godDrawSlot && player.jumpSlot > 0
            ? "jumpSlot"
...
function applyTurnStartMana(game, playerId) {
    const player = game.getPlayer(playerId);
...
function applyDrawPhaseEffects(game, playerId) {
    const player = game.getPlayer(playerId);
    const character = game.getCharacter(player.character);
...
export function performMulligan(game, playerId, indices) {
    const player = game.getPlayer(playerId);
...
export function buildTurnStartQueue(game, playerId) {
    const player = game.getPlayer(playerId);
...
export function processTurnStartQueue(game) {
    while (game.state.turnStartQueue.length) {
        const item = game.state.turnStartQueue[0];
...
export function finishStartTurn(game) {
    const player = game.getCurrentPlayer();
...
export function handlePendingChoice(game, payload) {
...
export function endTurn(game) {
    if (game.state.phase !== "mainTurn" && game.state.phase !== "combat")
        return false;

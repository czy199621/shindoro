...
function renderPrimaryResource(label: string, value: string, tone: "hp" | "mana"): string {
  return `
  ...
  return `
    <button class="${classes.join(" ")}" ${targetableHero ? `data-action="attack-hero" data-hero-id="${isEnemy ? "P2_hero" : "P1_hero"}"` : "disabled"}>
      ...
      <div class="slot-metrics">
        ${renderSlotMeter("跳脸槽", player.jumpSlot, "jump", "10 点可发动，13 点强制发动")}
        ${renderSlotMeter("神抽槽", player.godDrawSlot, "god", "10 点指定牌库，13 点指定备牌")}
      </div>
    </button>
  `;
}

import type { DeckConfig } from "../types.js";

function copies(cardId: string, count: number): string[] {
  return Array.from({ length: count }, () => cardId);
}

function validateDeck(mainDeck: string[]): void {
  if (mainDeck.length !== 50) {
    throw new Error(`Deck must contain 50 cards, received ${mainDeck.length}.`);
  }

  const counts = new Map<string, number>();
  for (const cardId of mainDeck) {
    const next = (counts.get(cardId) ?? 0) + 1;
    if (next > 3) {
      throw new Error(`Deck cannot contain more than 3 copies of ${cardId}.`);
    }
    counts.set(cardId, next);
  }
}

function defineDeck(mainDeck: string[], sideboard: string[]): DeckConfig {
  validateDeck(mainDeck);
  return { mainDeck, sideboard };
}

const PUBLIC_SIDEBOARD_FINISHERS = [
  "ouroboros_time_usurper",
  "michael_divine_executor",
  "chaos_imaginary_shadow",
  "shun_shadow_assassin",
  "justitia_absolute_judge"
];

export const STARTING_DECKS: Record<string, DeckConfig> = {
  character_a: defineDeck(
    [
      ...copies("ember_wolf", 3),
      ...copies("shrine_guard", 1),
      ...copies("novice_mage", 2),
      ...copies("dawn_healer", 1),
      ...copies("archivist_owl", 2),
      ...copies("blade_dancer", 3),
      ...copies("dusk_assassin", 2),
      ...copies("battlefield_bard", 2),
      ...copies("graveyard_dragger", 1),
      ...copies("storm_lancer", 3),
      ...copies("ashen_ranger", 3),
      ...copies("grave_knight", 1),
      ...copies("miracle_guardian", 1),
      ...copies("iron_colossus", 1),
      ...copies("tactical_insight", 3),
      ...copies("burn", 3),
      ...copies("arc_bolt", 2),
      ...copies("tactics_scroll", 2),
      ...copies("soul_shatter", 1),
      ...copies("truth_searchlight", 1),
      ...copies("burst_flame_lance", 3),
      ...copies("cinder_storm", 1),
      ...copies("judgment_beam", 1),
      ...copies("heavenly_thunder", 1),
      ...copies("time_usurpation", 1),
      ...copies("mirror_wall", 2),
      ...copies("ambush_sigil", 2),
      ...copies("war_banner", 1)
    ],
    PUBLIC_SIDEBOARD_FINISHERS
  ),
  character_b: defineDeck(
    [
      ...copies("shrine_guard", 3),
      ...copies("novice_mage", 3),
      ...copies("shield_doll", 2),
      ...copies("dawn_healer", 2),
      ...copies("landmine_girl", 2),
      ...copies("archivist_owl", 3),
      ...copies("day_off", 2),
      ...copies("graveyard_dragger", 2),
      ...copies("dusk_assassin", 1),
      ...copies("pact_weaver", 3),
      ...copies("mirror_sage", 2),
      ...copies("dorm_matron", 2),
      ...copies("grave_knight", 1),
      ...copies("miracle_guardian", 2),
      ...copies("iron_rice_bowl", 1),
      ...copies("top_donor", 1),
      ...copies("desperate_gamble", 3),
      ...copies("inspiration", 2),
      ...copies("healing_prayer", 2),
      ...copies("arc_bolt", 1),
      ...copies("soul_shatter", 1),
      ...copies("divine_intervention", 2),
      ...copies("wrath_of_god", 1),
      ...copies("heavenly_thunder", 2),
      ...copies("time_usurpation", 1),
      ...copies("underdog_shrine", 3)
    ],
    PUBLIC_SIDEBOARD_FINISHERS
  ),
  character_c: defineDeck(
    [
      ...copies("ember_wolf", 2),
      ...copies("shrine_guard", 2),
      ...copies("novice_mage", 3),
      ...copies("shield_doll", 2),
      ...copies("landmine_girl", 2),
      ...copies("archivist_owl", 2),
      ...copies("dawn_healer", 1),
      ...copies("blade_dancer", 2),
      ...copies("battlefield_bard", 3),
      ...copies("day_off", 3),
      ...copies("dusk_assassin", 1),
      ...copies("graveyard_dragger", 1),
      ...copies("storm_lancer", 2),
      ...copies("mirror_sage", 2),
      ...copies("dorm_matron", 2),
      ...copies("grave_knight", 3),
      ...copies("miracle_guardian", 2),
      ...copies("iron_rice_bowl", 2),
      ...copies("three_phase_plug", 1),
      ...copies("iron_colossus", 1),
      ...copies("top_donor", 1),
      ...copies("tactical_insight", 2),
      ...copies("tactics_scroll", 3),
      ...copies("war_banner", 3),
      ...copies("heavenly_thunder", 1),
      ...copies("time_usurpation", 1)
    ],
    PUBLIC_SIDEBOARD_FINISHERS
  ),
  character_d: defineDeck(
    [
      ...copies("ember_wolf", 1),
      ...copies("shrine_guard", 1),
      ...copies("novice_mage", 3),
      ...copies("landmine_girl", 2),
      ...copies("archivist_owl", 2),
      ...copies("dawn_healer", 1),
      ...copies("graveyard_dragger", 3),
      ...copies("dusk_assassin", 3),
      ...copies("day_off", 2),
      ...copies("mirror_sage", 2),
      ...copies("ashen_ranger", 2),
      ...copies("dorm_matron", 1),
      ...copies("grave_knight", 2),
      ...copies("miracle_guardian", 2),
      ...copies("iron_rice_bowl", 1),
      ...copies("iron_colossus", 2),
      ...copies("burn", 1),
      ...copies("arc_bolt", 3),
      ...copies("inspiration", 2),
      ...copies("soul_shatter", 3),
      ...copies("cinder_storm", 2),
      ...copies("burst_flame_lance", 2),
      ...copies("judgment_beam", 2),
      ...copies("wrath_of_god", 1),
      ...copies("meteor_fire_rain", 1),
      ...copies("heavenly_thunder", 1),
      ...copies("time_usurpation", 1),
      ...copies("ambush_sigil", 1)
    ],
    PUBLIC_SIDEBOARD_FINISHERS
  ),
  character_e: defineDeck(
    [
      ...copies("ember_wolf", 3),
      ...copies("shrine_guard", 1),
      ...copies("novice_mage", 2),
      ...copies("dawn_healer", 2),
      ...copies("landmine_girl", 1),
      ...copies("archivist_owl", 2),
      ...copies("blade_dancer", 2),
      ...copies("dusk_assassin", 2),
      ...copies("battlefield_bard", 2),
      ...copies("day_off", 2),
      ...copies("graveyard_dragger", 1),
      ...copies("storm_lancer", 3),
      ...copies("ashen_ranger", 3),
      ...copies("grave_knight", 2),
      ...copies("miracle_guardian", 1),
      ...copies("iron_rice_bowl", 1),
      ...copies("iron_colossus", 1),
      ...copies("tactical_insight", 2),
      ...copies("burn", 3),
      ...copies("arc_bolt", 2),
      ...copies("tactics_scroll", 2),
      ...copies("burst_flame_lance", 3),
      ...copies("mirror_wall", 3),
      ...copies("ambush_sigil", 1),
      ...copies("mana_drain", 1),
      ...copies("heavenly_thunder", 1),
      ...copies("time_usurpation", 1)
    ],
    PUBLIC_SIDEBOARD_FINISHERS
  ),
  character_f: defineDeck(
    [
      ...copies("shrine_guard", 3),
      ...copies("shield_doll", 3),
      ...copies("dawn_healer", 3),
      ...copies("landmine_girl", 3),
      ...copies("novice_mage", 1),
      ...copies("day_off", 3),
      ...copies("graveyard_dragger", 2),
      ...copies("dorm_matron", 3),
      ...copies("mirror_sage", 2),
      ...copies("pact_weaver", 1),
      ...copies("grave_knight", 3),
      ...copies("iron_rice_bowl", 2),
      ...copies("miracle_guardian", 1),
      ...copies("three_phase_plug", 2),
      ...copies("top_donor", 2),
      ...copies("desperate_gamble", 2),
      ...copies("healing_prayer", 2),
      ...copies("arc_bolt", 2),
      ...copies("mana_drain", 1),
      ...copies("soul_shatter", 2),
      ...copies("cinder_storm", 1),
      ...copies("wrath_of_god", 1),
      ...copies("meteor_fire_rain", 1),
      ...copies("heavenly_thunder", 1),
      ...copies("time_usurpation", 1),
      ...copies("underdog_shrine", 2)
    ],
    PUBLIC_SIDEBOARD_FINISHERS
  ),
  character_g: defineDeck(
    [
      ...copies("shrine_guard", 2),
      ...copies("novice_mage", 2),
      ...copies("shield_doll", 2),
      ...copies("dawn_healer", 2),
      ...copies("landmine_girl", 2),
      ...copies("archivist_owl", 3),
      ...copies("graveyard_dragger", 2),
      ...copies("day_off", 2),
      ...copies("dusk_assassin", 2),
      ...copies("mirror_sage", 3),
      ...copies("pact_weaver", 1),
      ...copies("dorm_matron", 2),
      ...copies("grave_knight", 2),
      ...copies("miracle_guardian", 1),
      ...copies("iron_rice_bowl", 1),
      ...copies("desperate_gamble", 3),
      ...copies("inspiration", 2),
      ...copies("healing_prayer", 2),
      ...copies("arc_bolt", 2),
      ...copies("mana_drain", 2),
      ...copies("soul_shatter", 2),
      ...copies("divine_intervention", 2),
      ...copies("judgment_beam", 1),
      ...copies("stagnant_hourglass", 2),
      ...copies("heavenly_thunder", 1),
      ...copies("time_usurpation", 1),
      ...copies("underdog_shrine", 1)
    ],
    PUBLIC_SIDEBOARD_FINISHERS
  )
};

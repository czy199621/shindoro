...
interface AiProfile {
  ...
  mulliganMaxCost: number;
  mulliganKeepIds: string[];
  keepRemovalCost: number;
  ...
  attackFace: number;
  trade: number;
  preserveMinion: number;
  godDrawHandThreshold: number;
  godDrawHpThreshold: number;
  preferredGodDrawIds: string[];
  preferredReserveIds: string[];
}
...
const REQUIRED_AI_TALENTS: Record<string, string[]> = {
  character_b: ["wide_grip", "vitality_ritual"]
};
...
const AI_PROFILES: Record<string, AiProfile> = {
  ...
  character_b: {
    ...DEFAULT_PROFILE,
    talentPriority: ["wide_grip", "vitality_ritual", "opening_insight", "desperate_recovery", "first_guardrail"],
    mulliganMaxCost: 4,
...
function getProfile(characterId: string): AiProfile {
  return AI_PROFILES[characterId] ?? DEFAULT_PROFILE;
}

function getPlayerProfile(state: GameState, playerId: PlayerId): AiProfile {
...
function countReadyAttackers(player: PlayerState): number {
  return player.board.filter((minion) => minion.canAttack).length;
}

function hasHandCard(player: PlayerState, ids: Set<string>): boolean {
  return player.hand.some((card) => ids.has(card.id));
}

function cardPriority(card: RuntimeCard, ids: string[]): number {
...
export function chooseAiTalentIds(characterId: string, seat: TalentSeat = "second"): string[] {
  ...
  const requiredTalents = REQUIRED_AI_TALENTS[characterId] ?? [];
  const priority = [
    ...requiredTalents,
    ...profile.talentPriority.filter((talentId) => !requiredTalents.includes(talentId)),
  ...
  for (const talentId of priority) {
    ...
    if ((counts.get(talent.id) ?? 0) >= talent.repeatLimit) continue;
...
function shouldKeepOpeningCard(state: GameState, playerId: PlayerId, card: RuntimeCard): boolean {
  const profile = getPlayerProfile(state, playerId);
  if (card.id === "coin") return true;
  if (profile.mulliganKeepIds.includes(card.id)) return true;
  if (card.currentCost <= profile.mulliganMaxCost && card.type === "minion") return true;
  if (card.currentCost <= profile.mulliganMaxCost && (card.type === "spell" || card.type === "trap")) {
    return !HEAL_IDS.has(card.id) || profile.heal >= 1.4;
  }
  if (REMOVAL_IDS.has(card.id) && card.currentCost <= profile.keepRemovalCost) return true;
  if (card.type === "persistent" && card.currentCost <= 4 && profile.persistent >= 1.8) return true;
  return false;
}

export function chooseAiMulliganIndices(state: GameState, playerId: PlayerId): number[] {
...
export function shouldAiUseJumpSlot(state: GameState, playerId: PlayerId): boolean {
  ...
  if (me.jumpSlot >= 12) return true;
  if (opp.hp <= myAttack + 3 && myAttack > 0) return true;
...
export function shouldAiUseGodDrawSlot(state: GameState, playerId: PlayerId): boolean {
  const me = state.players[playerId];
  const opp = state.players[getOpponentId(playerId)];
  const profile = getPlayerProfile(state, playerId);
...
function scoreMinionDamage(action: Extract<EffectAction, { type: "damage" }>, amount: number, opp: PlayerState, profile: AiProfile): number {
  ...
  if (action.target === "strongestEnemyMinion" || action.target === "weakestEnemyMinion") {
    const sorted = [...opp.board].sort((left, right) =>
      action.target === "strongestEnemyMinion" ? right.attack - left.attack : left.attack - right.attack
    );
...
function scoreHeal(me: PlayerState, amount: number, profile: AiProfile): number {
  const missingHp = Math.max(0, me.maxHp - me.hp);
  if (missingHp === 0) return 0.15 * amount;
  const urgency = me.hp <= 10 ? 1.8 : me.hp <= 14 ? 1.25 : 0.75;
  return Math.min(amount, missingHp) * profile.heal * urgency;
}

function scoreSlotGain(state: GameState, playerId: PlayerId, slot: "jump" | "godDraw", amount: number, profile: AiProfile): number {
  ...
  if (current < 10 && after >= 10) score += 4.5;
  if (current < 13 && after >= 13) score += 6.5;
  return score;
}

function scoreEffectAction(state: GameState, playerId: PlayerId, card: RuntimeCard, action: EffectAction): number {
  const me = state.players[playerId];
  const opp = state.players[getOpponentId(playerId)];
  const profile = getPlayerProfile(state, playerId);
  ...
  switch (action.type) {
    case "damage": {
      ...
      if (action.target === "enemyHero") {
        ...
        if (opp.hp <= amount + boardAttack(me)) score += 18;
        if (FACE_DAMAGE_IDS.has(card.id)) score += profile.faceDamage;
        return score;
      }
    ...
    case "buff": {
      const boardCount =
        action.target === "allFriendlyMinionsExceptSource" ? Math.max(0, me.board.length) : me.board.length;
...
function evaluateCardState(state: GameState, playerId: PlayerId, card: RuntimeCard, ignoreCost = false): number {
  const me = state.players[playerId];
  const opp = state.players[getOpponentId(playerId)];
  const profile = getPlayerProfile(state, playerId);
  ...
  if (HEAL_IDS.has(card.id)) score += me.hp <= 14 ? profile.heal : 0;
  if (DRAW_IDS.has(card.id)) score += me.hand.length <= 4 ? profile.draw : profile.draw * 0.45;
  if (JUMP_IDS.has(card.id)) score += profile.jumpSlot;
  if (GOD_DRAW_IDS.has(card.id)) score += profile.godDrawSlot;
  if (BOARD_BUFF_IDS.has(card.id) && me.board.length) score += profile.buff * me.board.length;
  if (GUARD_IDS.has(card.id)) score += profile.guard;
  ...
  if (pressure >= me.hp - 4) {
    if (REMOVAL_IDS.has(card.id)) score += 4;
    if (HEAL_IDS.has(card.id)) score += 3;
    if (card.tags?.includes("guard")) score += 2.5;
  }
...
function chooseBestCardId(cards: RuntimeCard[], state: GameState, playerId: PlayerId, reserve = false): string | null {
  ...
  const scored = cards
    .map((card) => {
      ...
      if (opp.board.length && REMOVAL_IDS.has(card.id)) score += 7;
      if (me.hp <= profile.godDrawHpThreshold && HEAL_IDS.has(card.id)) score += 7;
      if (opp.hp <= boardAttack(me) + 5 && FACE_DAMAGE_IDS.has(card.id)) score += 8;
      if (me.character === "character_f" && (JUMP_IDS.has(card.id) || GUARD_IDS.has(card.id))) score += 5;
      return { card, score };
    })
...
export function chooseAiDeckCardForGodDraw(state: GameState, playerId: PlayerId): string | null {
  return chooseBestCardId(state.players[playerId].deck, state, playerId, false);
}

export function chooseAiReserveCard(state: GameState, playerId: PlayerId): string {
...
function evaluateAttack(
  ...
  if (target.type === "hero") {
    ...
    if (opp.hp <= attacker.attack) score += 80;
    if (opp.hp <= attacker.attack + 4) score += 7;
    if (boardAttack(opp) >= me.hp && me.character !== "character_a") score -= 5;
  ...
  let score = 0;
  if (wouldKill) score += defenderValue * profile.trade + 2.5;
  if (!wouldKill) score += Math.min(attacker.attack, defender.health) * 0.65;
  if (wouldLose) score -= attackerValue * profile.preserveMinion;
  if (!wouldLose) score += attackerValue * 0.32;
  if (defender.attack >= me.hp) score += 12;
  if (defender.attack >= attacker.health && wouldKill) score += 2;
  if (defender.tags.includes("guard") && profile.attackFace > profile.trade) score += 1.5;
...
export function chooseAiAction(game: GameAiAdapter, playerId: PlayerId): AiDecision {
  ...
  if (!allActions.length) return { type: "endTurn" };

  const best = allActions[0];
  if (best.score < profile.actionThreshold) return { type: "endTurn" };
  return best;
}

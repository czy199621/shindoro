...
export function chooseAiDeckCardForGodDraw(state: GameState, playerId: PlayerId): string | null {
  ...
  if (opp.board.length) {
    const removal = me.deck.find((card) =>
      ["arc_bolt", "soul_shatter", "cinder_storm", "judgment_beam"].includes(card.id)
    );
    if (removal) return removal.id;
  }

  if (me.hp <= 10) {
    const healing = me.deck.find((card) =>
      ["healing_prayer", "divine_intervention", "grave_knight", "dawn_healer"].includes(card.id)
    );
    if (healing) return healing.id;
  }

  const draw = me.deck.find((card) =>
    ["inspiration", "novice_mage", "archivist_owl", "sage_archive"].includes(card.id)
  );
...
export function chooseAiReserveCard(state: GameState, playerId: PlayerId): string {
  ...
  if (opp.board.length >= 2) return "judgment_beam";
  if (me.hp <= 10) return "divine_intervention";
  return "miracle_guardian";
}

function evaluateCard(game: GameAiAdapter, playerId: PlayerId, card: RuntimeCard): number {
  ...
  if (card.type === "minion") {
    ...
    if (card.tags?.includes("rush")) score += 2.4;
    if (me.board.length >= 6) score -= 6;
  }

  if (card.type === "persistent") {
    score += (card.threat ?? 0) * 1.4;
    if (card.id === "sage_archive") score += 2.5;
    if (card.id === "underdog_shrine" && me.hp <= opp.hp) score += 3;
  }

  if (card.type === "trap") {
    score += 2.2;
    if (card.id === "mirror_wall" && opp.hand.some((item) => item.type === "spell")) score += 2;
  }
  ...
  for (const effect of card.effects ?? []) {
    ...
    if (action.type === "damage") {
      if (action.target === "enemyHero") {
        ...
        if (opp.hp <= action.amount + 3) score += 7;
        ...
        if (target) {
          ...
          if (target.health <= action.amount) score += target.attack + 2;
        }
      }
    }
...
function evaluateAttack(
  game: GameAiAdapter,
  playerId: PlayerId,
  attacker: GameAiAdapter["getReadyAttackers"] extends (playerId: PlayerId) => infer T ? T extends Array<infer U> ? U : never : never,
  target: AttackTarget
): number {
  const opp = game.getOpponent(playerId);
  if (target.type === "hero") {
    let score = attacker.attack * 1.4;
    if (opp.hp <= attacker.attack) score += 15;
    return score;
  }
  ...
  let score = 0;
  if (wouldKill) score += defender.attack + defender.health * 0.8 + 2;
  if (!wouldLose) score += attacker.attack * 0.5 + attacker.health * 0.4;
  if (wouldLose) score -= attacker.attack + attacker.health * 0.7;
  if (defender.attack >= attacker.health) score += 1.5;
...
export function chooseAiAction(game: GameAiAdapter, playerId: PlayerId): AiDecision {
  ...
  if (best.score < 0.9) return { type: "endTurn" };
  return best;
}
